...
In the New Session form, expand the MCP section to apply a saved profile or toggle individual servers before launch. Press `Ctrl+P` to save the current MCP selection as a new profile, then type a profile name and press Enter. Press `Ctrl+U` to update the active profile from the current selection.
